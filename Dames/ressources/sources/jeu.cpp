...
{
    *p_pion = p_tableau[p_depart_X][p_depart_Y];
    *p_temp_X = p_depart_X;
    *p_temp_Y = p_depart_Y;
    p_tableau[p_depart_X][p_depart_Y] = 0;
}
...
void poser_piece(int * p_pion, int * p_temp_X, int * p_temp_Y,
                 int ** p_tableau)
{
    p_tableau[*p_temp_X][*p_temp_Y] = *p_pion;
    *p_pion = 0;
}
...
{
    if(*p_pion != 0 && p_in->mousex < TAILLE_CARRE * NB_CARRE_X)
    {
        if(p_in->mousebuttons[SDL_BUTTON_LEFT])
        {
            poser_piece(p_pion, p_destination_X, p_destination_Y, p_tableau);
        }
        if(p_in->mousebuttons[SDL_BUTTON_RIGHT])
        {
            poser_piece(p_pion, p_origine_X, p_origine_Y, p_tableau);
        }
    }
    if(*p_pion == 0 && p_in->mousex < TAILLE_CARRE * NB_CARRE_X)
    {
